...
class TargetedAttack:
    ...
    def get_optimized_image(self):
        ...
        save_path = os.path.join(
            "figures",
            f"alpha_{self.alfa}clip_cutoff{self.theta_n_pct}_blur_size{self.theta_b_width}percentile{self.theta_c_pct}.png",
        )
...
def grid_search(
    ...
    for i in range(len(activation)):
        print(f"Iteration {i + 1}/{len(activation)}, Activation: {activation[i]}")
